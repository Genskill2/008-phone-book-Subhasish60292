...
void
print_usage (char *message, char *progname)
{
  printf ("Error : %s\n", message);
...
delete (FILE * db_file, char *name)
{
  ...
  if (strcmp (base->name, name) == 0)
    ...
    {
      while (base != NULL && strcmp (base->name, name) != 0)
	{
	  prev = base;
	  base = base->next;
	}
